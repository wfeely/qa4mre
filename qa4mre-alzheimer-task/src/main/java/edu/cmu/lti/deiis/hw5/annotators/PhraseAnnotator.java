...
public class PhraseAnnotator extends JCasAnnotator_ImplBase {
  ...
  public ArrayList<NounPhrase> extractNounPhrases(ArrayList<Token> tokenList, JCas jCas) {
    ...
    for (int i = 0; i < tokenList.size(); i++) {
      ...
      // Group5 changes: added adjective variants (JJR, JJS), noun variants (NNS, NNP, NNPS) and
      // determiners (DT)
      if (pos.startsWith("NN") || pos.startsWith("JJ") || pos.startsWith("CD")
              || pos.startsWith("JJR") || pos.startsWith("JJS") || pos.startsWith("NNS")
              || pos.startsWith("NNP") || pos.startsWith("NNPS") || pos.startsWith("DT")) {
        nounPhrase += word + " ";
      } else {
